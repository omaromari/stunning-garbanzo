"""
Greenscreen Area Detector v2 - Smart Detection
Intelligently identifies greenscreen-suitable areas while excluding furniture and obstacles.
"""
...
class SmartGreenscreenDetector:
    """
    Enhanced detector that excludes furniture and finds actual wall surfaces.
    """
    ...
    def analyze_image(self, image_path: str, 
        ...
        Args:
            ...
            max_area_ratio: Maximum zone size (60% of image) - prevents selecting entire image
        """
        print(f"\n{'='*60}")
        ...
        uniform_mask = self._detect_uniform_regions_strict(img_rgb)
        ...
        checked_zones = self._quality_check_zones(valid_zones, img_rgb, obstacle_mask)
    ...
    def _detect_walls(self, img: np.ndarray) -> np.ndarray:
        ...
        # (walls are typically large continuous areas)
    ...
    def _detect_uniform_regions_strict(self, img: np.ndarray) -> np.ndarray:
        """
        Strict uniformity detection - only very uniform surfaces.
        """
        # Convert to LAB color space (better for color difference)
        lab = cv2.cvtColor(img, cv2.COLOR_RGB2LAB)
        
        # Calculate local standard deviation in LAB space
        kernel_size = 25
        ...
        for channel in range(3):
            ...
            uniform_masks.append((std < 10).astype(np.uint8) * 255)
    ...
    def _extract_zones(self, mask: np.ndarray, img: np.ndarray) -> List[Dict]:
        ...
        for i, contour in enumerate(contours):
            ...
            # Compactness (closer to 1 = more compact/circular)
    ...
    def _quality_check_zones(self, zones: List[Dict], img: np.ndarray, 
                            obstacle_mask: np.ndarray) -> List[Dict]:
        """
        Perform quality checks on each zone.
        Rejects zones with too many obstacles or non-uniform color.
        """
        h, w = img.shape[:2]
        checked_zones = []
        ...
        for zone in zones:
            ...
            # Check 1: Obstacle coverage
            obstacle_ratio = np.sum(zone_obstacles > 0) / (zw * zh)
            if obstacle_ratio > 0.15:  # Reject if >15% covered by obstacles
                continue
            
            # Check 2: Color variance
            color_std = np.std(zone_img, axis=(0, 1)).mean()
            if color_std > 25:  # Too much color variation
                continue
            
            # Check 3: Shape reasonableness
            # Walls are typically rectangular or tall vertical surfaces
            ar = zone['aspect_ratio']
            if ar < 0.2 or ar > 8.0:  # Too extreme aspect ratio
                continue
            
            # Check 4: Not at extreme edges (usually doors/windows)
            center_x = x + zw/2
            center_y = y + zh/2
            
            # Reject if zone is within 5% of image edge
            margin = 0.05
            if (x < w * margin or (x + zw) > w * (1 - margin) or
                y < h * margin or (y + zh) > h * (1 - margin)):
                # Only reject if also small
                if zone['area'] < (w * h * 0.15):
                    continue
    ...
    def _rank_zones_v2(self, zones: List[Dict], img_w: int, img_h: int) -> List[Dict]:
        ...
        for zone in zones:
            ...
            # Prefer medium-large zones (15-50% of image)
            ...
            if 0.15 <= size_ratio <= 0.50:
                ...
                score += size_ratio * 200  # Small zones get lower score
            else:
                score += 20  # Very large zones penalized
            
            # Uniformity score (0-30 points)
            # Lower color std = better
            ...
            # Prefer rectangular shapes or vertical walls
            rect_score = zone['rectangularity'] * 10
            ar = zone['aspect_ratio']
            
            # Good aspect ratios: 16:9 (1.78), 4:3 (1.33), or tall (0.4-0.6)
            if 1.3 <= ar <= 2.0:
                rect_score += 5
            elif 0.3 <= ar <= 0.7:  # Tall wall
                rect_score += 4
    ...
    def visualize_results(self, result: Dict, output_path: str = None, 
        ...
        Args:
            ...
            show_top_n: Number of zones to show
            show_debug: If True, show additional debug info
        """
    ...
    def analyze_multiple_images(self, image_paths: List[str], 
                               output_dir: str = "greenscreen_results_v2") -> Dict:
        """Analyze multiple images."""
        output_dir = Path(output_dir)
        output_dir.mkdir(exist_ok=True)
        
        print("\n" + "="*60)
        print("GREENSCREEN DETECTOR V2 - SMART DETECTION")
        print("="*60)
